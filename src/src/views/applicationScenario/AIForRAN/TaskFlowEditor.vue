...
const canExecute = computed(() => {
  // 只检查数据加载模块：viavi-data 或 livenet-data 其中之一已配置即可
  const hasDataModule = flowNodes.value?.some(
    (node) =>
      (node.type === "viavi-data" || node.type === "livenet-data") &&
  ...
  return hasDataModule && !isExecuting.value;
});
...
const executeFlow = async () => {
  ...
  // 验证数据加载模块：viavi-data 或 livenet-data 其中之一已配置
  const dataModule = flowNodes.value.find(
    (node) =>
      (node.type === "viavi-data" || node.type === "livenet-data") &&
      node.status === "configured"
  );
  ...
  if (!dataModule) {
    ElMessage.warning("请配置数据加载模块（仿真数据加载或现网数据加载）");
    return;
  }
  ...
  try {
    // 获取数据加载模块配置（优先使用viavi-data）
    let dataConfig;
    let dataSource;

    const viaviNode = flowNodes.value.find(
      (n) => n.type === "viavi-data" && n.status === "configured"
    );
    const livenetNode = flowNodes.value.find(
      (n) => n.type === "livenet-data" && n.status === "configured"
    );

    if (viaviNode) {
      dataConfig = viaviNode.config;
      dataSource = "viavi";
    } else if (livenetNode) {
      dataConfig = livenetNode.config;
      dataSource = "livenet";
    }

    // 获取其他模块配置（可选，如果没有配置则使用默认值或跳过）
    const scenarioNode = flowNodes.value.find(
      (n) => n.type === "scenario-selector" && n.status === "configured"
    );
    const imageNode = flowNodes.value.find(
      (n) => n.type === "image-selector" && n.status === "configured"
    );
    const modelNode = flowNodes.value.find(
      (n) => n.type === "model-selector" && n.status === "configured"
    );

    const scenarioConfig = scenarioNode?.config || {
      selectedCategory: "",
      selectedSubOption: null,
    };
    const imageConfig = imageNode?.config || {
      selectedImageTypes: [],
      category: "",
      subOption: null,
    };
    const modelConfig = modelNode?.config || {
      useLargeModel: false,
      selectedSmallModel: "",
    };
    ...
    // 阶段2: 数据加载（根据数据源类型选择不同的加载方法）
    updateStageStatus("download", "running");
    if (dataSource === "viavi") {
      await downloadViaviData(dataConfig);
    } else {
      // 这里可以添加现网数据加载的逻辑
      await downloadLivenetData(dataConfig);
    }
    if (executionCanceled.value) return;
    ...
    // 检查是否需要生成图像
    if (imageNode && imageConfig.selectedImageTypes.length > 0) {
      // 有独立的图像选择模块配置
      ...
      imageGenParams = {
        ...
        downloadParams: dataConfig,
      };
      console.log("使用图像选择模块配置生成图像");
    } else if (dataSource === "livenet" && dataConfig.generateImages) {
      // livenet-data模块中启用了生成图像选项
      shouldGenerateImages = true;
      imageGenParams = {
        imageTypes: ["default"], // 使用默认图像类型
        category: dataConfig.scenario || "",
        subOption: dataConfig.aspect || "",
        dataSource: dataSource,
        downloadParams: dataConfig,
      };
      console.log("使用现网数据模块内置配置生成图像");
    }

    if (shouldGenerateImages) {
      updateStageStatus("image", "running");
      imageResult = await generateImages(imageGenParams);
      if (executionCanceled.value) return;
      updateStageStatus("image", "completed");
    } else {
      updateStageStatus("image", "completed");
      console.log("跳过图像生成：未启用图像生成选项");
    }
    ...
    if (
      modelNode &&
      (modelConfig.useLargeModel || modelConfig.selectedSmallModel)
    ) {
      // 有独立的模型选择模块配置
      ...
      console.log("使用模型选择模块配置进行分析");
    } else if (dataSource === "livenet" && dataConfig.enableAnalysis) {
      // livenet-data模块中启用了模型分析选项
      shouldRunAnalysis = true;
      ...
      analysisParams = {
        message: generateAnalysisMessage(defaultModelConfig),
        context: {
          dataSource: dataSource,
          downloadParams: dataConfig,
          category: dataConfig.scenario || "",
          subOption: dataConfig.aspect || "",
          imageTypes: shouldGenerateImages ? ["default"] : [],
          generatedImages: imageResult.images || [],
        },
        modelOptions: defaultModelConfig,
      };
      console.log("使用现网数据模块内置配置进行模型分析");
    }

    if (shouldRunAnalysis) {
      updateStageStatus("analysis", "running");
      analysisResult = await runModelAnalysis(analysisParams);
      if (executionCanceled.value) return;
      updateStageStatus("analysis", "completed");
    } else {
      updateStageStatus("analysis", "completed");
      console.log("跳过模型分析：未启用模型分析选项");
    }
    ...
    executionResult.value = {
      ...
      taskFlowConfig: {
        data: { type: dataSource, config: dataConfig },
        scenario: scenarioNode ? scenarioConfig : null,
        image: imageNode ? imageConfig : null,
        model: modelNode ? modelConfig : null,
      },
    };
...
const downloadLivenetData = async (config) => {
  console.log("加载现网数据:", config);
  ...
  try {
    ...
    a.href = url;
    a.download = `livenet_data.${config.format}`;
    document.body.appendChild(a);
    a.click();
    a.remove();
    window.URL.revokeObjectURL(url);

    console.log("现网数据下载成功");
    return { status: "success" };
  } catch (error) {
    console.error("现网数据下载失败:", error);
    throw new Error(`现网数据下载失败: ${error.message}`);
  }
};
